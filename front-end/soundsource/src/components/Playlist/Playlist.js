      ...
      axios.get("http://localhost:8080/suggested").then((response) => {
        if (response.data[0] != null) {
          ...
          for(var i in response.data) {
            if(response.data[i].playlistName == "General" && response.data[i].upvoteCount >= firstUpvote){
              firstId = response.data[i].id;
              firstUpvote = response.data[i].upvoteCount;
              firstArrayIndx = i;
            }
            if(response.data[i].playlistName == "Party" && response.data[i].upvoteCount >= secondUpvote){
              secondId = response.data[i].id;
              secondUpvote = response.data[i].upvoteCount;
              secondArrayIndx = i;
            }
            if(response.data[i].playlistName == "Working Out" && response.data[i].upvoteCount >= thirdUpvote){
